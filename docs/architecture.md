...
precipitation"). The gameplay mixin is taken only because no Fabric API hook covers the need: no
event or setter for per-call biome temperature; likewise the item-model-property registry is
private with no Fabric hook.
...
- **`season_clock`** (`item/FallowItems`) - a copper-and-redstone analogue of the vanilla clock
  ...
  - Registered into the **Tools &amp; Utilities** creative tab via `CreativeModeTabEvents`; crafted
    ...
    (`data/fallow/recipe/season_clock.json`). All the art is generated by `tools/season_clock_art.py`
...
on their `Consumable`. The `fallow:jam_fruits` tag lists all valid jam inputs and is open for
datapack extension.

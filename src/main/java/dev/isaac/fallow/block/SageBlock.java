...
/** Decorative wild sage placed by the forage spread task. No age, no interaction. */
public final class SageBlock extends BushBlock {
    // BushBlock.codec() returns MapCodec<BushBlock>; the cast is safe because this CODEC
    // only ever constructs SageBlock instances.
    @SuppressWarnings("unchecked")
    public static final MapCodec<BushBlock> CODEC =
        (MapCodec<BushBlock>) (MapCodec<?>) simpleCodec(SageBlock::new);

    public SageBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemStack getCloneItemStack(LevelReader level, BlockPos pos, BlockState state,
            boolean includeData) {
        return new ItemStack(FallowItems.SAGE);
    }
}

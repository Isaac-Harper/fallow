...
/** Decorative wild ramsons (wild garlic) placed by the forage spread task. No age, no interaction. */
public final class RamsonsBlock extends BushBlock {
    // BushBlock.codec() returns MapCodec<BushBlock>; the cast is safe because this CODEC
    // only ever constructs RamsonsBlock instances.
    @SuppressWarnings("unchecked")
    public static final MapCodec<BushBlock> CODEC =
        (MapCodec<BushBlock>) (MapCodec<?>) simpleCodec(RamsonsBlock::new);

    public RamsonsBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemStack getCloneItemStack(LevelReader level, BlockPos pos, BlockState state,
            boolean includeData) {
        return new ItemStack(FallowItems.RAMSONS);
    }
}

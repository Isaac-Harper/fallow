...
/** Decorative wild sorrel placed by the forage spread task. No age, no interaction. */
public final class SorrelBlock extends BushBlock {
    // BushBlock.codec() returns MapCodec<BushBlock>; the cast is safe because this CODEC
    // only ever constructs SorrelBlock instances.
    @SuppressWarnings("unchecked")
    public static final MapCodec<BushBlock> CODEC =
        (MapCodec<BushBlock>) (MapCodec<?>) simpleCodec(SorrelBlock::new);

    public SorrelBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemStack getCloneItemStack(LevelReader level, BlockPos pos, BlockState state,
            boolean includeData) {
        return new ItemStack(FallowItems.SORREL);
    }
}
